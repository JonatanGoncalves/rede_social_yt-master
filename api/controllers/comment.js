...
export const creatComment = (req, res) => {
    ...
    db.query("INSERT INTO comments SET ? ", {comment_desc, post_id,comment_user_id},(error)=>{
        if(error) {
            console.log(error);
            return res.status(500).json({msg:"Aconteceu algum erro no servidor, tesnte novamente mais tarde"}) 
        } else {
...
export const getComment = (req, res) => {
    db.query("SELECT c.*, u.username, userImg FROM comments AS c JOIN user AS u ON (u.id = c.comment_user_id) WHERE post_id = ? ORDER BY created_at DESC", 
    [req.query.post_id],
    (error, data) => {
        if(error){
            console.log(error);
