...
function Post(props:{post:IPost}) {
    ...
    const likesMutation = useMutation({
        mutationFn:async (newLikes: {}) => {
            if(liked) {
                await makeRequest.delete(`likes/?likes_post_id=${id}&likes_user_id=${user?.id}`).then((res) =>{
                    setLiked(false);
                    return res.data;
                });
            } else {
        ...
        <div className="w-full bg-white rounded-lg p-4 shadow-md">
            <header className="flex gap-2 pb-4 border-b items-center">
              <Link href={'/profile?id=' + userId}>
                ...
                {img && <img className="rounded-lg w-full" src={ `./upload/${img}` } alt="imagem do post"/>}
                <div className="flex justify-between py-4 border-b">
                    ...
                    <button onClick={() => setShowComments(!showComments)}>
                            {commentQuery.data &&
                                ...
                                `${commentQuery.data?.length} comentarios`}
                    </button>
                </div>            
